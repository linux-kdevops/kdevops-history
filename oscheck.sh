...
if [ "$ONLY_INSTALL_DEPS" == "true" ]; then
	OSCHECK_SETUP_SYSTEM="true"
fi
...
install_req()
{
	req_install_possible $1
	if [ $? -eq 0 ] ; then
		${OSCHECK_ID}_install_$1
	fi
}
...
# OSCHECK_SETUP_SYSTEM and ensure you have a proper
...
check_services()
{
	...
	if [ $? -eq 0 ]; then
		 ...
		 if [ $? -ne 0 ] ; then
			if [ "$OSCHECK_SETUP_SYSTEM" = "true" ]; then
...
check_reqs()
{
	...
	for dir in $DIRS_NEEDED; do
		if [ ! -d $dir ]; then
			echo "ERROR: you must create $dir"
			if [ "$OSCHECK_SETUP_SYSTEM" = "true" ]; then
				mkdir -p $dir
			fi
			RET=1
...
oscheck_include_os_files()
{
	...
	if [ ! -f $OSCHECK_INCLUDE_FILE ]; then
		if [ "$OSCHECK_SETUP_SYSTEM" == "true" ]; then
			echo "Your distribution lacks $OSCHECK_INCLUDE_FILE file, consider implementing to install deps ..."
		fi
	fi
}
...
oscheck_has_test_dev_handle()
{
	if [ ! -e $OS_FILE ]; then
		return 1
	fi
	declare -f ${OSCHECK_ID}_test_dev_setup > /dev/null;
	return $?
}

oscheck_run_test_dev_handle()
{
	oscheck_has_test_dev_handle $OSCHECK_ID
	if [ $? -eq 0 ] ; then
		${OSCHECK_ID}_test_dev_setup
	fi
}
