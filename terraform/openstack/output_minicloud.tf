...
data "null_data_source" "group_hostnames_and_ports_v2" {
  ...
  inputs = {
    value = "${format("%30s  :  %s%s%03d%s ", replace(urlencode(element(split("name: ", element(data.yaml_list_of_strings.list.output, count.index)), 1)), "%7D", ""), "ssh debian@minicloud.parqtec.unicamp.br -p ", element(split(".", element(openstack_compute_instance_v2.fstests_instances.*.access_ip_v4, count.index)), 2), ceil(element(split(".", element(openstack_compute_instance_v2.fstests_instances.*.access_ip_v4, count.index)), 3)), local.ssh_key_i)}"
  }
}
